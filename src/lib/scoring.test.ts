...
describe('reputationSubScore', () => {
  it('scales a flat 8/10 across all metrics to 80/100', () => {
    expect(reputationSubScore(flatReputation, DEFAULT_WEIGHTS.reputation)).toBeCloseTo(80);
  });
  ...
  it('returns 0 when all weights are 0', () => {
    ...
    (Object.keys(zero) as (keyof ReputationScores)[]).forEach((k) => (zero[k] = 0));
...
describe('coverageSubScore', () => {
  ...
  it('reflects partial coverage proportionally to points', () => {
    ...
    const single: CoverageFeatures = { ...noCoverage, floodCoverIncluded: true };
...
describe('rankInsurers', () => {
  ...
  it('ranks by price-to-value ratio, best value first', () => {
    ...
    const ranked = rankInsurers([high, low], { high: 2000, low: 500 }, DEFAULT_WEIGHTS);
    expect(ranked[0].insurer.id).toBe('low');
    expect(ranked[0].ratio).toBeGreaterThan(ranked[1].ratio!);
  });

  it('prefers the better-value insurer when prices are equal', () => {
    const ranked = rankInsurers([low, high], { high: 1500, low: 1500 }, DEFAULT_WEIGHTS);
    expect(ranked[0].insurer.id).toBe('high');
  });

  it('excludes blank quotes from ratio ranking but keeps them after priced ones', () => {
    const ranked = rankInsurers([high, low], { high: null, low: 1000 }, DEFAULT_WEIGHTS);
    expect(ranked[0].insurer.id).toBe('low');
